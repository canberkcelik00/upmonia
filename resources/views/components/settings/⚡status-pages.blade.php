...
<div>
    ...
    @if ($creating)
        <x-ui.form-section :title="__('app.status_pages_new')" :description="__('app.status_pages_new_description')">
            <x-slot:actions>
                <x-ui.button type="submit" form="status-page-form" variant="primary">{{ __('app.status_pages_publish') }}</x-ui.button>
                <x-ui.button type="button" variant="ghost" wire:click="cancel">{{ __('app.cancel') }}</x-ui.button>
            </x-slot:actions>
            <form id="status-page-form" wire:submit="save" class="grid max-w-[440px] gap-3.5">
                ...
                <x-ui.field :label="__('app.status_pages_slug')" error="slug">
                    <div class="flex items-center rounded-control border border-line-strong text-sm">
                        <span class="px-3 text-muted">{{ url('/durum') }}/</span>
                        <input wire:model="slug" type="text" class="w-full rounded-r-md bg-transparent px-1 py-2 text-ink focus:outline-none">
                    </div>
                </x-ui.field>
        ...
        @forelse ($pages as $page)
            <tr wire:key="page-{{ $page->id }}">
                <td>
                    ...
                    <a href="{{ route('status-page.show', $page->slug) }}" target="_blank" class="text-xs text-muted hover:text-ink hover:underline">/durum/{{ $page->slug }}</a>
                </td>
                <td class="text-ink-2">{{ $page->monitors_count }}</td>
                <td>
                ...
                <td class="text-right text-xs">
                    <button wire:click="toggleEnabled({{ $page->id }})" class="font-medium text-ink-2 hover:text-ink">
                        {{ $page->enabled ? __('app.status_pages_unpublish') : __('app.status_pages_publish') }}
                    </button>
                    <button wire:click="delete({{ $page->id }})" wire:confirm="{{ __('app.maintenance_delete_confirm') }}" class="ml-3 font-medium text-down-text hover:underline">{{ __('app.delete') }}</button>
                </td>
            </tr>
        @empty
            <tr>
