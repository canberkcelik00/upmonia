{{-- Shared by monitors.form (create) and monitors.show (edit-in-place) — plain Blade
     ...
     component included it. One card, not sectioned — this form doesn't carry enough
     fields to justify splitting it up. $editing (bool) tells the two callers apart: it
     locks the type select and swaps the submit label / cancel action. --}}
...
<x-ui.card class="max-w-xl">
    <div class="grid gap-3.5">
        ...
        <details class="group -mx-5 rounded-none border-x-0 border-b-0 border-line">
            <summary class="flex cursor-pointer list-none items-center justify-between border-t border-line px-5 py-3.5 text-sm text-ink-2 select-none">
                {{ __('app.field_advanced') }}
                <x-phosphor-caret-down class="size-4 text-faint transition-transform duration-150 group-open:rotate-180" />
            </summary>

            <div class="grid gap-3.5 border-t border-line px-5 pt-3.5">
    ...
    <div class="mt-6 flex items-center gap-3">
        <x-ui.button type="submit" variant="primary" wire:loading.attr="disabled">
            {{ $editing ? __('app.save') : __('app.monitor_create_submit') }}
        </x-ui.button>
        @if ($editing)
