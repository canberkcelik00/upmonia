...
return [
    ...
    'allow_private_targets' => env('UPTIK_ALLOW_PRIVATE_TARGETS', false) && ! app()->environment('production'),
    ...
    'probe' => [
        'batch_size' => (int) env('UPTIK_PROBE_BATCH_SIZE', 60),
        'concurrency' => (int) env('UPTIK_PROBE_CONCURRENCY', 20),
        'lock_seconds' => (int) env('UPTIK_PROBE_LOCK_SECONDS', 50),
    ],
    ...
    'ssrf' => [
        'http_ports' => array_values(array_unique(array_merge(
            [80, 443, 8080, 8443],
            array_filter(array_map('intval', explode(',', (string) env('UPTIK_EXTRA_HTTP_PORTS', ''))))
        ))),
        'tcp_ports' => array_values(array_unique(array_merge(
            ...
            array_filter(array_map('intval', explode(',', (string) env('UPTIK_EXTRA_TCP_PORTS', ''))))
        ))),
    ],
